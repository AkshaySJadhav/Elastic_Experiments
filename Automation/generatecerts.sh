...
openssl req -passin pass:"password" -subj "/C=US/ST=Random/L=Random/O=Global Security/OU=IT Department/CN=Local Certificate"  -x509 -new -nodes -key rootCA.key -sha256 -days 1024 -out rootCA.pem
...
openssl x509 -req -passin pass:"password" -in es01.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out es01.crt -days 500 -sha256 -extfile  <(printf "subjectAltName=DNS:localhost,DNS:es01")
...
openssl x509 -req -passin pass:"password" -in kib01.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out kib01.crt -days 500 -sha256 -extfile  <(printf "subjectAltName=DNS:localhost,DNS:kib01")

VERSION="8.8.0" HOSTCERTSDIR="$PWD" ESCERTSDIR="/usr/share/elasticsearch/config" KIBCERTSDIR="/usr/share/kibana/config" docker-compose -f docker-compose-SSL-cluster.yml up
